# Лабораторная 3. Первый контейнер
...
Что бы его установить на NixOS необходимо добавить соответствующее название пакета - `docker` или конкретную версию, например, `docker_29` в список зависимостей системы и выполнить:
...
Выполнил команду создания контейнера:
...
- `bash` - это название процессов который станет PID 1 - самым приоритетным в контейнере.
...
Данный файл оказался именно по этому пути в результате инструкции `COPY ./site/ /var/www/html/` внутри **DockerFile**.
